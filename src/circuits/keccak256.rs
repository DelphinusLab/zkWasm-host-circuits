...
mod tests {
    ...
    #[test]
    fn test_keccak_circuit_01() {
        let mut hasher = host::keccak256::KECCAK_HASHER.clone();
        let input = [0u32;16].map(|_| Fr::zero()).to_vec();
        hasher.update(&input);
        let result = hasher.squeeze();
        let mut inputs = [0u32;17].map(|_| Fr::zero()).to_vec();
    ...
    #[test]
    fn test_keccak_circuit_02() {
        let mut hasher = host::keccak256::KECCAK_HASHER.clone();
        let input = [0u32;16].map(|_| Fr::one()).to_vec();
        hasher.update(&input);
        let result = hasher.squeeze();
        let mut inputs = [0u32;17].map(|_| Fr::one()).to_vec();
        inputs[16] = Fr::from_u128(1 << 63) + Fr::one();
        let test_circuit = TestCircuit { inputs, result };
        println!("result is {:?}", result);
        let prover = MockProver::run(17, &test_circuit, vec![]).unwrap();
    ...
    #[test]
    fn test_keccak_circuit_03() {
        let mut hasher = host::keccak256::KECCAK_HASHER.clone();
        let input = [0u32;2].map(|_| Fr::one()).to_vec();
        hasher.update(&input);
        let result = hasher.squeeze();
        let mut inputs = [0u32;17].map(|_| Fr::one()).to_vec();
        for x in 3..16 {
    ...
    #[test]
    fn test_keccak_circuit_04() {
        let mut hasher = host::keccak256::KECCAK_HASHER.clone();
        let mut input = [0u32;17].map(|_| Fr::one());
        for x in 3..16 {
            input[x] = Fr::zero();
        }
        input[16] = Fr::from_u128(1 << 63);
        let result = hasher.update_exact(&input);
        let mut inputs = [0u32;17].map(|_| Fr::one()).to_vec();
        for x in 3..16 {
            inputs[x] = Fr::zero();
        }
