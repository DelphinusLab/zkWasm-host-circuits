...
impl<F: FieldExt> KeccakArithChip<F> {
    ...
    pub fn configure(cs: &mut ConstraintSystem<F>) -> KeccakArithConfig {
        ...
        cs.lookup_any("xor validity check", |meta| {
            ...
            let pow4 = constant_from!(1u64<<4);

            vec![(lhs * pow8 + rhs * pow4 + res, table)]
        });
        ...
        cs.create_gate("limb acc constraint", |meta| {
            ...
            vec![
                sel.clone() * (acc_lhs.clone() - lhs - acc_n_lhs * constant_from!(1u64<<4) * sel_n.clone()),
                sel.clone() * (acc_rhs.clone() - rhs - acc_n_rhs * constant_from!(1u64<<4) * sel_n.clone()),
                sel.clone() * (acc_res.clone() - res - acc_n_res * constant_from!(1u64<<4) * sel_n.clone()),
    ...
    pub fn assign_value_xor (
        ...
        for _ in 0..sz {
            ...
            let limb_lhs = bn_lhs.modpow(&BigUint::from(1u128), &BigUint::from(1u128<<4));
            let limb_rhs = bn_rhs.modpow(&BigUint::from(1u128), &BigUint::from(1u128<<4));
            let limb_res = bn_res.modpow(&BigUint::from(1u128), &BigUint::from(1u128<<4));

            bn_lhs = (bn_lhs - limb_lhs.clone()).div(BigUint::from(1u128<<4));
            bn_rhs = (bn_rhs - limb_rhs.clone()).div(BigUint::from(1u128<<4));
            bn_res = (bn_res - limb_res.clone()).div(BigUint::from(1u128<<4));
        ...
        for i in 0..sz {
            ...
            let a = self.config.assign_cell(region, self.offset, &KeccakArithConfig::acc_lhs(), acc_lhs)?;
            dbg!(a.value);
            self.config.assign_cell(region, self.offset, &KeccakArithConfig::res(), limb_res)?;
            self.config.assign_cell(region, self.offset, &KeccakArithConfig::acc_res(), acc_res)?;
        ...
        self.config.assign_cell(region, self.offset, &KeccakArithConfig::res(), F::zero())?;
        ...
        self.config.assign_cell(region, self.offset, &KeccakArithConfig::acc_res(), F::zero())?;
    ...
    pub fn initialize(
        ...
        for i in 0..1<<4 {
            for j in 0..1<<4 {
                let res = i ^ j;
                let table = self.config.assign_cell(
                    region,
                    i*(1<<4)+j,
        ...
        fn synthesize(
            ...
            layouter.assign_region(
                ...
                |mut region| {
                    ...
                    //dbg!(keccak_arith_chip.offset);
                    // assign helper
                    let mut offset = 0;
                    //helper_chip.assign_value(&mut region, &mut offset, v)?;
                    let a = helper_chip.assign_lhs(&mut region, &mut offset, lhs)?;
                    dbg!(a.value());
                    //helper_chip.assign_rhs(&mut region, &mut offset, rhs)?;
                    //keccak_arith_chip.config.get_advice_column(KeccakArithConfig::acc_lhs())
                    Ok(())
                }
            )?;
            Ok(())
