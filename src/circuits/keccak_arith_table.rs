...
impl<F: FieldExt> KeccakArithChip<F> {
    ...
    pub fn assign_value_xor (
        ...
        for i in 0..sz {
            ...
            let acc_r = self.config.assign_cell(region, self.offset, &KeccakArithConfig::acc_rhs(), acc_rhs)?;
            //dbg!(acc_l.value);
            dbg!(acc_r.value);
            //dbg!(acc_re.value);
            let rem = self.config.assign_cell(region, self.offset, &KeccakArithConfig::rem(), F::from_u128((sz-i) as u128))?;
            dbg!(rem.value);
