...
use crate::circuits::bls::{assigned_cells_to_bn381, enable_fq_permute};
use crate::utils::Limb;
use halo2ecc_s::circuit::base_chip::BaseChipConfig;
use halo2ecc_s::circuit::base_chip::{BaseChip, BaseChipOps};
...
use crate::adaptor::map2curveadaptor::MAP2CURVE_LIMBS_SIZE;

fn get_fp2_from_cells(
    gseccc: &mut GeneralScalarEccContext<G1Affine, Fr>,
    a: &Vec<Limb<Fr>>,
) -> AssignedFq2<Fq, Fr> {
    let x_re = assigned_cells_to_bn381(&a, 0);
    let x_im = assigned_cells_to_bn381(&a, 4);
    let u: AssignedFq2<Fq, Fr> = (
        gseccc.base_integer_ctx.assign_w(&x_re),
        gseccc.base_integer_ctx.assign_w(&x_im),
    );
    u
}

//manually mark the cell permute and output the cell after assignment.
fn mark_fq2_permute(gseccc: &mut GeneralScalarEccContext<G1Affine, Fr>, fq2: &AssignedFq2<Fq, Fr>) {
    for i in 0..4 {
        gseccc
            .native_ctx
            .borrow_mut()
            .enable_permute(&fq2.0.limbs_le[i]);
        gseccc
            .native_ctx
            .borrow_mut()
            .enable_permute(&fq2.1.limbs_le[i]);
    }
}
...
impl<F: FieldExt> Map2CurveChip<F> {
    pub fn new(config: Map2CurveChipConfig) -> Self {
        Map2CurveChip {
            ...
            range_chip: RangeChip::new(config.range_chip_config),
            point_select_chip: SelectChip::new(config.select_chip_config),
            _marker: PhantomData,
        }
    }

    pub fn configure(meta: &mut ConstraintSystem<F>) -> Map2CurveChipConfig {
        let base_chip_config = BaseChip::configure(meta);
        let select_chip_config = SelectChip::configure(meta);
        let range_chip_config = RangeChip::configure(meta);
        Map2CurveChipConfig {
            base_chip_config,
            range_chip_config,
            select_chip_config,
    ...
    pub fn load_map2curve_circuit(
        &mut self,
        arg_cells: &Vec<Limb<Fr>>,
        layouter: &mut impl Layouter<Fr>,
    ) -> Result<(), Error> {
        let context = Rc::new(RefCell::new(Context::new()));
        let mut gseccc = GeneralScalarEccContext::<G1Affine, Fr>::new(context);
        let mut inputs = vec![];
        let mut outputs = vec![];
        // arg_cells format  (2 + 2 + 2)*4
        for arg_group in arg_cells.chunks_exact(MAP2CURVE_LIMBS_SIZE) {
            let u = get_fp2_from_cells(&mut gseccc, &arg_group.to_vec());
            inputs.push(u.clone());

            let (x, y) = simplified_swu_in_context(&mut gseccc, &u);
            let x = gseccc.fq2_reduce(&x);
            let y = gseccc.fq2_reduce(&y);
            mark_fq2_permute(&mut gseccc, &x);
            mark_fq2_permute(&mut gseccc, &y);
            outputs.push((x, y));
        }

        let records = Arc::try_unwrap(Into::<Context<Fr>>::into(gseccc).records)
            .unwrap()
            .into_inner()
            .unwrap();
        layouter.assign_region(
            || "base",
            |mut region| {
                let cells = records.assign_all_opt(
                    &mut region,
                    &self.base_chip,
                    &self.range_chip,
                    ...
                    Some(cells) => {
                        ...
                        inputs.iter().enumerate().for_each(|(i, x)| {
                            enable_fq_permute(
                                ...
                                &cells,
                                &x.0,
                                &arg_cells[MAP2CURVE_LIMBS_SIZE * i..MAP2CURVE_LIMBS_SIZE * i + 4]
                                    .to_vec(),
                            )
                            .unwrap();
                            enable_fq_permute(
                                ...
                                &x.1,
                                &arg_cells
                                    [MAP2CURVE_LIMBS_SIZE * i + 4..MAP2CURVE_LIMBS_SIZE * i + 8]
                                    .to_vec(),
                            )
                            .unwrap();
                        });
                        //output (x,y)
                        outputs.iter().enumerate().for_each(|(i, (x, y))| {
                            enable_fq_permute(
                                &mut region,
                                &cells,
                                &x.0,
                                &arg_cells
                                    [MAP2CURVE_LIMBS_SIZE * i + 8..MAP2CURVE_LIMBS_SIZE * i + 12]
                                    .to_vec(),
                            )
                            .unwrap();
                            enable_fq_permute(
                                ...
                                &x.1,
                                &arg_cells
                                    [MAP2CURVE_LIMBS_SIZE * i + 12..MAP2CURVE_LIMBS_SIZE * i + 16]
                                    .to_vec(),
                            )
                            .unwrap();
                            enable_fq_permute(
                                ...
                                &y.0,
                                &arg_cells
                                    [MAP2CURVE_LIMBS_SIZE * i + 16..MAP2CURVE_LIMBS_SIZE * i + 20]
                                    .to_vec(),
                            )
                            .unwrap();
                            enable_fq_permute(
                                ...
                                &y.1,
                                &arg_cells
                                    [MAP2CURVE_LIMBS_SIZE * i + 20..MAP2CURVE_LIMBS_SIZE * i + 24]
                                    .to_vec(),
                            )
                            .unwrap();
                        });
...
pub fn simplified_swu_in_context(
...

    {
        ...
        let sgn0_u_does_not_equal_sgn0_y: AssignedFq2<Fq, Fr> =
            (sgn0_u_minus_sgn0_y_squared, zero_i);
...
fn simplified_swu_outputs_correct_test_vector() {
    ...
    println!(
        "{:?}",
        gseccc.base_integer_ctx.ctx.borrow_mut().range_offset
    );
}
