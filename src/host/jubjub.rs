...
mod tests {
    ...
    pub fn verify_alt_jubjub_signature() {
        ...
        let pk_x = BigUint::parse_bytes(b"139f1d319d2a51a1938aef20ae4aa05b4bacef0c95ec2acf6d70b0430bed7808", 16).unwrap();
        let pk_y = BigUint::parse_bytes(b"023abdc9dac65b2e858cf258c0a9b0c2c8a83a86ec2ebbaab8fdb5169b262597", 16).unwrap();
        let pk = Point {
            x: bn_to_field(&pk_x),
            y: bn_to_field(&pk_y),
        };
        ...
        let sig_rx = BigUint::parse_bytes(b"00d711880dcccc0767dad1aa321fa2f54462c0d91e7c708836b5ac274215e4ca", 16).unwrap();
        let sig_ry = BigUint::parse_bytes(b"303438ab520086fb5e723bdc3c5e0f6a99b7d1caca0b8871ce16ab467d4baf5c", 16).unwrap();
        ...
        println!("sig_rx is: {:?} {:?}", BigUint::to_u64_digits(&sig_rx), sig_rx);
        println!("sig_ry is: {:?} {:?}", BigUint::to_u64_digits(&sig_ry), sig_ry);

        let base_x = BigUint::parse_bytes(b"2ef3f9b423a2c8c74e9803958f6c320e854a1c1c06cd5cc8fd221dc052d76df7", 16).unwrap();
        let base_y = BigUint::parse_bytes(b"05a01167ea785d3f784224644a68e4067532c815f5f6d57d984b5c0e9c6c94b7", 16).unwrap();
        let p_g = Point {
        ...
        println!("base x is: {:?} {:?}", BigUint::to_u64_digits(&base_x), BigUint::to_u64_digits(&field_to_bn(&-bn_to_field::<Fr>(&base_x))));
        println!("base y is: {:?} {:?}", BigUint::to_u64_digits(&base_y), base_y);
