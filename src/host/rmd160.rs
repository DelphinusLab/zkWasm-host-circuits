...
fn compress(w: &Vec<u32>, values: Vec<u32>) -> Vec<u32> {
    ...
    for ((idxs,shift), offset) in O.zip(R).zip(ROUNDS_OFFSET) {
        for limb_index in 0..16 {
            ...
            println!("rol {} is {:?}, value: {}, offset {}, shift {}", round, rol1, values[idx], offset, shift[idx]);
            rol_modifier(round, &mut rol1, values[idx], offset, shift[idx]);
            println!("rol-after {} is {:?}", round, rol1);
        }
        round += 1;
    }
    round = 0;
...
mod tests {
    ...
    macro_rules! process_block(
        ($h:ident, $data:expr,
         ...
         $( par_round5: h_ordering $pf0:expr, $pf1:expr, $pf2:expr, $pf3:expr, $pf4:expr;
            ...
            println!("round 1 {} {} {} {} {}", bb[0], bb[1], bb[2], bb[3], bb[4]);
            ...
            println!("{} {} {} {} {}", bb[0], bb[1], bb[2], bb[3], bb[4]);
    ...
    fn test_rmd160_compress() {
        ...
        let mut b = super::INITBUF.clone();
        let r1 = compress(&mut b, &[0u8; 64]);
        println!("{:?}", r);
        println!("{:?}", b);
    }
}
